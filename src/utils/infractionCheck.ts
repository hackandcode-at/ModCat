import { EmbedBuilder, Guild, GuildMember, TextBasedChannel, TextChannel, User } from "discord.js";
import { db } from "./database";
import keys from "../keys";

export const infractionCheck = (member: GuildMember, reply: TextBasedChannel, mod: User, guild: Guild) => {
    db.all(`SELECT * FROM infractions WHERE memberId = ${member.id} ORDER BY createdAt DESC`, [], async (err, rows: any) => {
        ...
        if (last24Hours > 3 || last7Days > 5) {
            ...
            const reason = `Automatischer Ban wegen zu vieler Maßnahmen. (ID: ${infractionId})`
            ...
            // Return true to indicate that the user has been banned
            return true
        }
    })
}